...
const searchResults = computed(() => {
  ...
  return maps.filter((map) =>
    [map.name, map.subtitle, map.description].some((text) =>
      String(tt(text)).toLocaleLowerCase().includes(normalizedQuery.value),
    ),
  )
})

const getHighlightSegments = (text) => {
  const source = String(text ?? '')
  const keyword = query.value.trim()

  if (!source || !keyword) {
    return [{ text: source, highlighted: false, offset: 0 }]
  }
  ...
  const segments = []
  let cursor = 0
  let matchIndex = normalizedSource.indexOf(normalizedKeyword)
    ...
    <div v-if="normalizedQuery" class="grid gap-3">
      ...
      <div v-if="searchResults.length" class="grid max-h-[52vh] gap-3 overflow-y-auto pr-2">
        <button
          ...
          :key="map.id"
          type="button"
          class="group flex w-full items-center gap-4 border-2 border-[#172018] bg-[#f8fbf2] p-4 text-left shadow-[4px_4px_0_#172018] transition-[transform,box-shadow,background-color] duration-75 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:bg-[#eff8d8] hover:shadow-[6px_6px_0_#172018] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#172018] dark:border-[#d8d3c5] dark:bg-[#252826] dark:shadow-[4px_4px_0_#59645c] dark:hover:bg-[#2d332c] dark:hover:shadow-[6px_6px_0_#59645c] dark:focus-visible:outline-[#f2f1e8]"
          @click="openResult(map)"
        >
          <span
            class="grid size-11 shrink-0 place-items-center border-2 border-[#172018] bg-[#d8f461] text-[#172018] shadow-[3px_3px_0_#172018] dark:shadow-[3px_3px_0_#59645c]"
          >
            <MapPinned class="size-5" stroke-width="2.5" />
          </span>

          <span class="min-w-0 flex-1">
            <span class="block truncate font-mono text-base font-black">
              <template
                v-for="segment in getHighlightSegments(tt(map.name))"
                :key="`${segment.offset}-${segment.highlighted}`"
              >
                <mark
                  v-if="segment.highlighted"
                  class="box-decoration-clone bg-[#ffd75b] px-[1px] text-[#172018] dark:bg-[#d8f461]"
                >
                  {{ segment.text }}
                </mark>
            ...
            <span class="mt-1 block truncate text-xs font-black text-[#637067] dark:text-[#b7beb8]">
              <template
                v-for="segment in getHighlightSegments(tt(map.subtitle))"
                :key="`${segment.offset}-${segment.highlighted}`"
              >
                <mark
                  v-if="segment.highlighted"
            ...
            >
              <template
                v-for="segment in getHighlightSegments(tt(map.description))"
                :key="`${segment.offset}-${segment.highlighted}`"
              >
                <mark
                  v-if="segment.highlighted"
